...
public class AccountCalculatorTest {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < acc.length; i++) {
			System.out.println("계좌번호 : " + acc[i].getName() + " / 잔액 : " + acc[i].getBalance() + " / 이율 : " + acc[i].getInterestRate() + "%");
		}
		...
		for (int i = 0; i < acc.length; i++) {
			System.out.println("계좌번호 : " + acc[i].getName() + " / 이율 : " + acc[i].getInterestRate() + " / 잔액 : " + (acc[i].getBalance() + acc[i].calculateInterest()));
		}
	}
